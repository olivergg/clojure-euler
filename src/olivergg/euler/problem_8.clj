...
(def stringified10000digits (apply str (str/split-lines big1000digits)))

(defn parseint
  "parse a string to an integer (compatible with char)"
  [s]
  (Integer/parseInt (re-find #"\A-?\d+" (str s)))
  )

(defn productstring
  [initlist]
  (loop [inputlist initlist,
         out 1]
    (let [[head & tail] inputlist]
      (if (nil? tail)
        (* out (parseint (str head)))
        (recur tail (* out (parseint (str head))))
        )
      )
    )
  )
